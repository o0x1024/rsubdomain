use pnet::datalink;
use pnet::packet::dns::{DnsPacket, MutableDnsPacket, MutableDnsQueryPacket};
...
fn send_by_datalink() {
    ...
    let ipv4_destination: Ipv4Addr = Ipv4Addr::new(10,1,172,76);
...
fn send_icmp_datalink() {
    ...
    let interface = interfaces
        .iter()
        .find(|iface| iface.name == "ens33" && !iface.is_loopback())
        .expect("No suitable network interface found");
                        ...
                        match ipv4_packet.get_next_level_protocol() {
                            IpNextHeaderProtocols::Udp => {
                                ...
                                if udp_packet.get_source() == 53
                                    || udp_packet.get_destination() == 53
                                {
                                    println!(
                                        "recv dns packet: sourse port: {}, dest port: {} ",
                                    ...
                                    let mut ethernet_buffer =
                                        vec![0u8; ethernet_packet.packet().len()];
                                    let mut new_ethernet_packet =
                                        MutableEthernetPacket::new(&mut ethernet_buffer).unwrap();

                                    // 手动复制以太网帧字段
                                    new_ethernet_packet
                                        .set_destination(ethernet_packet.get_source());
                                    new_ethernet_packet
                                        .set_source(ethernet_packet.get_destination());
                                    new_ethernet_packet
                                        .set_ethertype(ethernet_packet.get_ethertype());
                                    ...
                                    new_ipv4_packet
                                        .set_header_length(ipv4_packet.get_header_length());
                                    new_ipv4_packet.set_dscp(ipv4_packet.get_dscp());
                                    new_ipv4_packet.set_ecn(ipv4_packet.get_ecn());
                                    new_ipv4_packet
                                        .set_total_length(ipv4_packet.get_total_length());
                                    new_ipv4_packet
                                        .set_identification(ipv4_packet.get_identification());
                                    new_ipv4_packet.set_flags(ipv4_packet.get_flags());
                                    new_ipv4_packet
                                        .set_fragment_offset(ipv4_packet.get_fragment_offset());
                                    new_ipv4_packet.set_ttl(ipv4_packet.get_ttl());
                                    new_ipv4_packet.set_next_level_protocol(
