...
impl SendDog {
    ...
    pub fn send(&self, domain: String, dnsname: String, src_port: u16, flag_id: u16) {
        ...
        let udpchk = ipv4_checksum(&udp_header.to_immutable(), &ipv4_source, &ipv4_destination);
        udp_header.set_checksum(udpchk);
        ...
        ipv4_header.set_dscp(1);
        ipv4_header.set_ecn(0);
        ipv4_header.set_total_length(total_length);
        ipv4_header.set_identification(21482);
        ipv4_header.set_next_level_protocol(IpNextHeaderProtocols::Udp);

        ipv4_header.set_flags(0);
        ipv4_header.set_fragment_offset(0);
        ipv4_header.set_source(ipv4_source);
        ipv4_header.set_destination(Ipv4Addr::from_str(dnsname.as_str()).unwrap());
        ipv4_header.set_ttl(200);
